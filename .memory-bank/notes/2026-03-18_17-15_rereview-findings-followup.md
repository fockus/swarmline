...
- lazy fail-fast optional exports создали новый public-surface regression: `from swarmline.runtime import *` падает без SDK extras, потому что SDK-only names остаются в `__all__`.
- аналогичный public-surface regression есть в `swarmline.skills`: YAML-only helpers попали в `__all__`, и star import теперь требует PyYAML даже для core registry/types.
