...
**Файл:** `src/cognitia/runtime/structured_output.py`
...
**Файл:** `src/cognitia/agent/tool.py`
...
```python
from cognitia.agent.tool import tool, ToolDefinition
```
...
**Файл:** `src/cognitia/runtime/registry.py`
...
**Файл:** `src/cognitia/runtime/cancellation.py`
...
**Файл:** `src/cognitia/runtime/types.py`
...
**Файл:** `src/cognitia/runtime/base.py`
...
Монолитный `protocols.py` разбит на модули в `src/cognitia/protocols/`:
...
Backward-compatible: `from cognitia.protocols import MessageStore` работает как раньше через `__init__.py`.

`RuntimePort` помечен deprecated — новый код использует `AgentRuntime` из `cognitia.runtime.base`.
...
- `cognitia.protocols` re-exports не изменились
