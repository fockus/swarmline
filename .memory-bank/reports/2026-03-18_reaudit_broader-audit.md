...
2. builtin `cli` добавлен в valid runtime names/registry, но legacy fallback в `RuntimeFactory.create()` остаётся несовместимым:
   - `src/swarmline/runtime/registry.py:141-145`
   - `src/swarmline/runtime/factory.py:181-192`
3. `swarmline.runtime` public surface регрессировал для optional SDK exports:
   - `src/swarmline/runtime/__init__.py:59-102`
   - `src/swarmline/runtime/__init__.py:153-173`

### Downgraded after broader audit

4. finding про `swarmline.skills.__all__` нужно трактовать осторожнее:
   - `src/swarmline/skills/__init__.py:15-21`
...
- `src/swarmline/runtime/claude_code.py:223-229`
...
- `src/swarmline/runtime/deepagents.py:221-227`
...
- `src/swarmline/orchestration/workflow_executor.py:43-54`
...
- `src/swarmline/orchestration/workflow_executor.py:88-96`
...
падает не на ожидаемом `ImportError("YamlSkillLoader ...")`, а раньше — через cold import `swarmline` и `yaml` в `runtime.model_registry`.
...
- `docs/why-swarmline.md:34-49`
- код:
  - `src/swarmline/runtime/capabilities.py:8`
  - `src/swarmline/runtime/registry.py:141-145`
...
- код:
  - `src/swarmline/hooks/__init__.py:13-26`
...
Package docstring говорит, что `YamlSkillLoader` вынесен в infrastructure layer и `skills` должен быть “чистый registry без IO”. Но docs и tests продолжают подавать loader как каноничную часть core `swarmline.skills` surface.
...
- source hot cluster:
  - `src/swarmline/runtime/deepagents.py:11-45` → `11` нарушений `E402`
- test-side шум:
...
- `src/swarmline/runtime/structured_output.py:25,30`
- `src/swarmline/runtime/options_builder.py:131`
- `src/swarmline/runtime/thin/llm_providers.py:167,213-229`
...
- dual-path session state `adapter + runtime + runtime_messages` остаётся в hot path:
  ...
  - `src/swarmline/session/manager.py:270-348`
- lifecycle API `Agent.cleanup()` в основном ceremonial:
  - `src/swarmline/agent/agent.py:27`
  - `src/swarmline/agent/agent.py:94-99`
...
- `DeepAgentsSubagentOrchestrator` по умолчанию остаётся tool-incomplete path:
  - `src/swarmline/orchestration/deepagents_subagent.py:32` и соседний ownership slice
...
2. README / API reference / runtimes / why-swarmline docs sync for `cli`
