...
- current working tree in `/Users/fockus/Apps/swarmline`
...
- File: `src/swarmline/session/manager.py:324-327`
...
- File: `src/swarmline/runtime/registry.py:141-145`
...
### 3. `swarmline.runtime` optional exports are no longer package-level import safe

- File: `src/swarmline/runtime/__init__.py:165-170`
- Impact: `from swarmline.runtime import *` now aborts in SDK-free environments because `__all__` still advertises SDK-only names while `__getattr__` raises `ImportError`.
- Proof: reproduced with blocked `claude_agent_sdk`; star import fails on `ClaudeOptionsBuilder` / `RuntimeAdapter`.

### 4. `swarmline.skills` now exports YAML-only helpers unconditionally

- File: `src/swarmline/skills/__init__.py:15-21`
- Impact: `from swarmline.skills import *` now requires PyYAML even though the core registry/types are dependency-free.
...
- Files:
  - `src/swarmline/__init__.py:10-42`
  - `src/swarmline/runtime/model_registry.py:18`
- Severity: P1 audit gap / pre-existing contract violation
- Impact:
  - cold `import swarmline` fails if `yaml` is absent or blocked;
  - cold `from swarmline.skills import YamlSkillLoader` fails before `skills.__getattr__` can produce the new fail-fast message.
- Proof:
  - reproduced `import swarmline` under blocked `yaml` → `ModuleNotFoundError: import of yaml halted; None in sys.modules`
  - reproduced `from swarmline.skills import YamlSkillLoader` under blocked `yaml` from a cold interpreter → same raw `ModuleNotFoundError`
- Interpretation:
...
- Proof:
  - reproduced `from swarmline.hooks import registry_to_sdk_hooks` under blocked `claude_agent_sdk` → `ImportError`
  - reproduced `hasattr(swarmline.hooks, "registry_to_sdk_hooks")` under blocked `claude_agent_sdk` → `ImportError`

### A3. Runtime docs still advertise only three runtimes although `cli` is public and registered

- Files:
  - `docs/runtimes.md:3-17`
  - `docs/why-swarmline.md:49`
...
- Proof:
  ...
  - `docs/why-swarmline.md` says “All three runtimes”
  - code and examples expose `cli` via `src/swarmline/runtime/cli/__init__.py`, `src/swarmline/runtime/registry.py`, `examples/19_cli_runtime.py`
...
- Files:
  ...
  - `README.md:301-307`
  - `src/swarmline/skills/__init__.py:1-4`
...
- Impact:
  - the test deletes only `swarmline.skills*`, not the top-level `swarmline` package;
  - if earlier tests already imported `swarmline`, the test passes and gives false confidence;
...
- The largest buckets are:
  - `E402` module import ordering in `src/swarmline/runtime/deepagents.py` and several tests
...
- `src/swarmline/runtime/__init__.py`, `src/swarmline/hooks/__init__.py` and `src/swarmline/skills/__init__.py` now intentionally fail fast, but standard Python capability probing via `hasattr()` is no longer safe for those symbols. This is not necessarily wrong, but it should be treated as an explicit API decision and documented as such.
- `tests/unit/test_skills.py` still imports `YamlSkillLoader` from `swarmline.skills`, which reinforces the old public-surface story and makes it harder to migrate the loader cleanly to an application-only seam.
...
2. Decouple cold `import swarmline` from `yaml` so optional import behavior becomes real, not only warm-process behavior.
