...
@pytest.mark.parametrize("mode,expected", [("auto", True), ("native", True), ("prompt", False)])
def test_use_native_tools_derived_from_structured_mode(mode: str, expected: bool) -> None:
...
def test_explicit_use_native_tools_overrides_derive() -> None:
    cfg = AgentConfig(
        system_prompt="x", runtime="thin", structured_mode="auto", use_native_tools=False
    )
...
def test_build_runtime_config_path_matches_wiring() -> None:
    cfg = AgentConfig(system_prompt="x", runtime="thin", structured_mode="auto", max_turns=15)
