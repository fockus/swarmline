...
Usage::
    ...
    swarmline-daemon start [--config config.yaml] [--pid-path path] [--port 8471]
    swarmline-daemon stop [--pid-path path]
    swarmline-daemon status [--host host] [--port 8471]
    swarmline-daemon pause [--host host] [--port 8471]
    swarmline-daemon resume [--host host] [--port 8471]
...
_DEFAULT_PORT = 8471
_DEFAULT_PID = "~/.swarmline/daemon.pid"


def main(argv: list[str] | None = None) -> None:
    """CLI entry point."""
    parser = argparse.ArgumentParser(
        prog="swarmline-daemon",
        description="Swarmline Daemon — long-running process manager",
    )
