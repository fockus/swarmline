"""swarmline init — scaffold a new Swarmline agent project.

Usage:
    swarmline init my-agent
    swarmline init my-agent --runtime claude --memory sqlite
    swarmline init my-agent --full
    swarmline init my-agent --output ./projects
    swarmline init my-agent --force
"""
...
_AGENT_PY = Template(
    '''\
"""$project_name — Swarmline AI Agent.
...
stack = SwarmlineStack.from_config("config.yaml")
...
_CONFIG_YAML_MINIMAL = Template(
    """\
# Swarmline Agent Configuration — $project_name
# Docs: https://swarmline.readthedocs.io/
...
_CONFIG_YAML_FULL = Template(
    """\
# Swarmline Agent Configuration — $project_name (full)
# Docs: https://swarmline.readthedocs.io/
...
runtime:
  type: $runtime
  model: auto         # resolved via SWARMLINE_MODEL or runtime default
  max_tokens: 4096
...
# SWARMLINE_MODEL=claude-3-5-sonnet-latest

# Memory (only needed for sqlite backend)
# SWARMLINE_DB_PATH=./$project_slug.db
"""
)
...
AI agent built with [Swarmline](https://swarmline.readthedocs.io/).
...
- [Swarmline Documentation](https://swarmline.readthedocs.io/)
- [Examples](https://swarmline.readthedocs.io/examples/)
"""
)
...
services:
  agent:
    ...
    volumes:
      - ./data:/app/data
    environment:
      - SWARMLINE_DB_PATH=/app/data/$project_slug.db
...
See: https://swarmline.readthedocs.io/skills/
"""
)
...
def _scaffold_project(
    ...
    extra_deps: list[str] = []
    if runtime == "claude":
        extra_deps.append('    "swarmline[claude]",')
    elif runtime == "deepagents":
        extra_deps.append('    "swarmline[deepagents]",')
    if memory_effective == "sqlite":
        extra_deps.append('    "swarmline[sqlite]",')
    extra_deps_str = "\n".join(extra_deps)
...
def init_command(
    ...
    """Scaffold a new Swarmline agent project.
